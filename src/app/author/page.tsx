...
export default function AuthorPage() {
  ...
  const [bundlePreview, setBundlePreview] = useState<{
    ...
    metadata?: { author?: string; sourceDocuments?: string; moduleMapping?: string; checksum?: string; signature?: string };
  } | null>(null);
  const [importHistory, setImportHistory] = useState<{ importedAt: string; checksum: string; importedCount: number; signatureValid: boolean }[]>([]);
  ...
  function addCheckpoint() {
    setDraft((prev) => ({
      ...
      checkpoints: [
        ...prev.checkpoints,
        { ...blankCheckpoint, id: `cp-${prev.checkpoints.length + 1}` }
      ]
    }));
  }
    ...
    <main className="mx-auto max-w-4xl space-y-4 p-6">
      ...
      <section className="space-y-2 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <textarea
          ...
          rows={8}
          placeholder="Paste source text here..." aria-label="Source document text"
          value={sourceText}
            ...
            <ul className="mt-2 space-y-1">
              {importHistory.map((entry) => (
                <li key={`${entry.importedAt}-${entry.checksum}`}>
                  {entry.importedAt}: {entry.importedCount} cases, checksum {entry.checksum}, signature {entry.signatureValid ? 'valid' : 'invalid'}
                </li>
              ))}
            </ul>
          </div>
      ...
      <form className="space-y-4 rounded-lg bg-white p-4 shadow-sm" onSubmit={handleSubmit}>
        ...
        <textarea
          className="w-full rounded border border-slate-300 p-2"
          rows={3}
          placeholder="Presentation summary" aria-label="Presentation summary"
          value={draft.presentation}
          onChange={(event: ChangeEvent<HTMLTextAreaElement>) => setDraft((prev) => ({ ...prev, presentation: event.target.value }))}
        />
      ...
      {message ? <p className="rounded border border-slate-200 bg-slate-50 p-3 text-sm" role="status" aria-live="polite">{message}</p> : null}
    </main>
  );
}
